...
int f(int n,vector<int>&dp,vector<int>&heights){
    ...
    int first=f(n-1,dp,heights)+abs(heights[n]-heights[n-1]);
    int second=INT_MAX;
    if(n>1) second=f(n-2,dp,heights)+abs(heights[n]-heights[n-2]);
...
for (int i = 2; i <n ; ++i)
{
    int first=abs(heights[i]-heights[i-1])+dp[i-1];
    int second=abs(heights[i]-heights[i-2])+dp[i-2];
...
for (int i = 1; i <n ; ++i)
{
    li first=prev1+abs(heights[i]-heights[i-1]);
    li second=X;
    if(i>1)second=prev2+abs(heights[i]-heights[i-2]);
    li current=min(first,second);
    prev2=prev1;
    prev1=current;
}
ct<<prev1<<el;
