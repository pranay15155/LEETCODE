...
li find_parent(li node,vec &parent){
	if (node==parent[node])
	{
		return node;
	}
...
void union_by_size(li u,li v,vec &parent, vec &size){
	li pu=find_parent(u,parent);
	li pv=find_parent(v,parent);
	if (pu!=pv)
	{
		if (size[pu]>=size[pv])
		{
...
void pranay_prasoon(){
	...
	vvi f(n),g(n);
	vep store;
	vec parentf(n,0),sizef(n,1);
	vec parentg(n,0),sizeg(n,1);
	...
	for(int i=0;i<n;i++){
		for(auto &x:g[i]){
			union_by_size(i,x,parentg,sizeg);
	...
	// for(int i=0;i<n;i++){
	// 	for(auto &x:f[i]){
	// 		union_by_size(i,x,parentf,sizef);
	// 	}
	// }
	...
	for (int i = 0; i < m1; ++i)
	{
		...
		li pug=find_parent(u,parentg);
		li pvg=find_parent(v,parentg);

		// li puf=find_parent(u,parentf);
		// li pvf=find_parent(v,parentf);

		if (pug==pvg)
		{
			union_by_size(u,v,parentf,sizef);
		}
		else ans++;
	}
	...
	for (int i = 0; i < n; ++i)
	{
		li pu=find_parent(i,parentf);
		li pv=find_parent(i,parentg);
		stf.insert(pu);
		stg.insert(pv);
	}
	// d3(stf,stg,ans)
	li k=stf.size()-stg.size();
	if(k>0)ans+=k;
	ct<<ans<<el;
	d(parentf)
	d(parentg)
